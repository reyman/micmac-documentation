...
Cette fonction `generate-MobileGroup` renvoie une population égale ou inférieure à la population indiqué par la taille de l'avion `gairplane-size`. C'est en effet à ce moment là que la stratégie individuelle est utilisé, si elle est activé (`gstrategy3-riskCulture` = `true`).  
