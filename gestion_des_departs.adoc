...
Cette fonction `generate-MobileGroup` renvoie une population égale ou inférieure à la population indiquée par la taille de l'avion `gairplane-size`. C'est en effet à ce moment là que la stratégie individuelle est utilisé, si celle-ci est activé (`gstrategy3-riskCulture` = `true`).  
