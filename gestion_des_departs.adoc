...
Cette fonction `generate-MobileGroup` renvoie une population égale ou inférieure à la population indiqué par la taille de l'avion `gairplane-size`. C'est en effet à ce moment là que la stratégie individuelle se decide si elle est activé.  
